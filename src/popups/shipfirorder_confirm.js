...
function Shipfirorder_confirm(props) {

  const confirm = async (event) => {
    event.preventDefault();
    const formdata = new FormData(event.target);
    const volume = formdata.get('volume');
    props.onConfirm(volume);
  }
            ...
            <div className="p-2 flex-fill bg-info">
              <div className="row m-0">
                ...
                <div className="col col-sm-8 input-group-prepend">
                  <label className="input-group-text">{props.viewshipfitorderselected.evetypename}</label>
                </div>
              </div>
              <div className="row m-0">
                <div className="col col-sm-4 input-group-prepend">
...
export default Shipfirorder_confirm;
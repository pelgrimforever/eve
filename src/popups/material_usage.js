...
/*    } else if(selectedmaterial.name==sort_drone) {
  ...
  const usematerial = async (event) => {
    event.preventDefault();
    const formdata = new FormData(event.target);
    const amountused = formdata.get('amount');
    if(amountused>0 && material!==null) {
      props.onMaterialuse(material, amountused);
    }
  }
  ...
  return (
    <Modal show={props.show} size="lg" id="tradelinemodal" centered>
        <Form onSubmit={usematerial}>
      <ModalHeader className="bg-dark">
        <ModalTitle className="text-light">Used material</ModalTitle>
      </ModalHeader>
      <ModalBody>
              ...
              <div className="mx-auto bg-light p-1">
                <div className="d-flex">
                  <div className="p-2 flex-fill bg-info">
                    <div className="row m-0">
                      <div className="col col-sm-12 d-flex">
                        <span className="mx-2" style={{width:'150px'}}>amount used</span>
